	...
	editCourseBtns[i].addEventListener("click", (e) => {
		...
		if (editCourseBtns[i].classList.contains("edit")) {
			...
			function getCodeSelect(courses) {
				...
				codeSelect.setAttribute("name", "codeSelect");
			...
			function getGradeSelect() {
				...
				gradeSelect.setAttribute("name", "gradeSelect");
			...
			termSelect.setAttribute("name", "termSelect");
			termSelect.innerHTML =
				'<option value="Term" disabled selected hidden> Choose a term ...</option>\n' +
				termOptions;
			termValue[i].textContent = "";
